...
| lockout\_time\_period | The time period, in minutes, during which consecutive failed login attempts are counted for lockout purposes. The range is from 1 minutes to 720 minutes. The default is 5 minutes. | `number` | `5` | no |
...
| pwd\_strength\_check | Cisco recommends using a strong password enforcement. The default value is true. | `string` | `"yes"` | no |
