...
variable "auth_protocol" {
  description = "Defines the authentication protocol to use with the RADIUS server.  Options are chap, mschap, pap.  The default is pap."
  type        = string
  default     = "pap"
  validation {
...
variable "key" {
  description = "The secret text string shared between the device and a specific RADIUS server."
  type        = string
  sensitive   = true
}
...
variable "monitor_user" {
  description = "Username to use for testing RADIUS server functionality."
  type        = string
  default     = "default"
}

variable "monitor_pwd" {
  description = "Password to use for testing RADIUS server functionality."
  type        = string
  sensitive   = true
  default     = ""
}
...
variable "port" {
  description = "The service port number for the RADIUS service. The range is from 1 to 65535. The default is 1812; another common value is also 1645."
  type        = number
  default     = 1812
  validation {
    condition = (
...
variable "retries" {
  description = "The number of retries when contacting the RADIUS endpoint. The range is from 0 to 5 retries. The default 1."
  type        = number
  default     = 1
  validation {
    condition = (
...
variable "timeout" {
  description = "The timeout for communication with a RADIUS provider server. The range is from 1 to 60 seconds. The default is 5 seconds. If set to 0, the AAA provider timeout is used."
  type        = number
  default     = 5
  validation {
    condition = (
